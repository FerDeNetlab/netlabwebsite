...
export async function GET(request: Request) {
  ...
  try {
    ...
    // ========== Aportes de Capital ==========
    const aportesAll = await sql`SELECT * FROM aportes_capital ORDER BY fecha DESC` as Record<string, unknown>[]
    const aportesMes = aportesAll.filter(a => {
      const f = new Date(a.fecha as string)
      return f.getMonth() + 1 === mes && f.getFullYear() === anio
    })
    const aportesMesTotal = aportesMes.reduce((s, a) => s + Number(a.monto || 0), 0)

    const aportesPorSocio: Record<string, number> = {}
    for (const a of aportesAll) {
      const s = a.socio as string
      aportesPorSocio[s] = (aportesPorSocio[s] || 0) + Number(a.monto)
    }
    const sociosAportes = Object.entries(aportesPorSocio).map(([nombre, total]) => ({ nombre, total })).sort((a, b) => b.total - a.total)
    ...
    return NextResponse.json({
      ...
      aportes_capital: {
        total_global: aportesAll.reduce((s, a) => s + Number(a.monto || 0), 0),
        total_mes: aportesMesTotal,
        socios: sociosAportes,
        movimientos_mes: aportesMes.map(a => ({ socio: a.socio, monto: Number(a.monto), concepto: a.concepto, fecha: a.fecha })),
      },
    })
  } catch (error) {
