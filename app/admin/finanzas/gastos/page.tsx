...
import { ArrowLeft, Search, ChevronLeft, ChevronRight, X, FileText, FileCode2, TrendingDown, Tag, Pencil, Save, RefreshCw } from 'lucide-react'
...
export default function GastosPage() {
    ...
    const fetchMovimientos = () => {
        setLoading(true)
        fetch(`/api/finanzas/movimientos?tipo=cargo&mes=${mes}&anio=${anio}`)
            .then(r => r.json())
    ...
    const filtered = movimientos.filter(m => {
        ...
        return m.descripcion.toLowerCase().includes(q) ||
            ...
            (m.emisor_nombre || '').toLowerCase().includes(q) ||
            (m.referencia || '').toLowerCase().includes(q)
    })
    const totalCargos = filtered.reduce((s, m) => s + Number(m.monto), 0)
    ...
    const cfdisFiltrados = cfdisDisp.filter(c => {
        ...
        return (c.emisor_nombre || '').toLowerCase().includes(q) ||
                        ...
                        <div className="p-6 space-y-6">
                            <div className="flex items-center justify-between border-b border-red-500/20 pb-4">
                                <div>
                                    ...
                                    <p className="text-gray-400 font-mono text-sm mt-1">Cargos del estado de cuenta bancario</p>
                                </div>
                            </div>

                            <div className="flex gap-1 border-b border-red-500/20">
                                <button onClick={() => setTabPrincipal('cargos')}
                                    className={`px-4 py-2 font-mono text-xs border-b-2 -mb-px transition-colors ${tabPrincipal === 'cargos' ? 'border-red-400 text-red-400' : 'border-transparent text-gray-500 hover:text-gray-300'}`}>
                                    <TrendingDown className="h-3 w-3 inline mr-1" /> Cargos del mes
                                </button>
                                <button onClick={() => setTabPrincipal('cfdis')}
                                    className={`px-4 py-2 font-mono text-xs border-b-2 -mb-px transition-colors ${tabPrincipal === 'cfdis' ? 'border-red-400 text-red-400' : 'border-transparent text-gray-500 hover:text-gray-300'}`}>
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                <div className="bg-zinc-900/60 border border-red-500/20 rounded-lg px-3 py-2 font-mono">
                                    <div className="text-[10px] text-gray-500">total cargos</div>
                                    <div className="text-red-400 text-sm font-bold">{fmt(totalCargos)}</div>
                                </div>
                            ...
                            <div className="relative">
                                ...
                                <input type="text" placeholder="Buscar por descripcion, etiqueta, proveedor..."
                            ...
                            {movimientos.length === 0 ? (
                                <div className="text-center py-12">
                                    ...
                                    <p className="font-mono text-gray-400">No hay cargos en {MESES[mes - 1]} {anio}</p>
                                    <p className="font-mono text-gray-600 text-sm mt-1">Sube tu estado de cuenta en <a href="/admin/finanzas/conciliacion" className="text-red-400 hover:underline">Banco</a></p>
                                </div>
                            ) : filtered.length === 0 ? (
                                                ...
                                                <tr key={m.id} className="hover:bg-zinc-800/30 transition-colors">
                                                    ...
                                                    <td className="p-3 min-w-[140px]">
                                                        {editingId === m.id ? (
                                                            ...
                                                            <div>
                                                                {m.etiqueta
                                                                    ? <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] bg-red-500/10 text-red-400 border border-red-500/20"><Tag className="h-2.5 w-2.5" />{m.etiqueta}</span>
                                                    ...
                                                    <td className="p-3 text-right space-x-1 whitespace-nowrap">
                                                        {editingId === m.id ? (
                                                            <>
                                                                <button onClick={() => saveEdit(m.id)} className="text-yellow-400 hover:text-yellow-300"><Save className="h-3.5 w-3.5 inline" /></button>
                                                                <button onClick={() => setEditingId(null)} className="text-gray-500 hover:text-gray-300"><X className="h-3.5 w-3.5 inline" /></button>
                                                            </>
                                                        ) : (
                                                            <button onClick={() => startEdit(m)} className="text-gray-600 hover:text-yellow-400 transition-colors"><Pencil className="h-3.5 w-3.5 inline" /></button>
                                                        )}
                                                    </td>
                                                </tr>
                                            ))}
                                ...
                                <div className="space-y-4">
                                    <div className="flex gap-2 items-center">
                                        ...
                                        <button onClick={fetchCfdisTab}
                                            className="px-3 py-2 font-mono text-xs bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 text-red-400 rounded transition-colors flex items-center gap-1">
                                            ...
                                            <table className="w-full font-mono text-xs">
                                                ...
                                                <tbody className="divide-y divide-red-500/5">
                                                    {cfdisTab
                                                        .filter(c =>
                                                            !searchCfdiTab.trim() ||
                                                            (c.emisor_nombre || '').toLowerCase().includes(searchCfdiTab.toLowerCase()) ||
                                                            c.emisor_rfc.toLowerCase().includes(searchCfdiTab.toLowerCase()) ||
                    ...
                    <div className="p-5 space-y-4">
                        <div className="flex items-center justify-between border-b border-red-500/20 pb-3">
                            <h2 className="font-mono text-sm text-red-400 flex items-center gap-2">
                                <FileText className="h-4 w-4" /> ligar XML / CFDI recibido
                            </h2>
                        ...
                        <div className="bg-zinc-900/60 border border-red-500/10 rounded-lg p-4 font-mono">
                            ...
                            <div className="mt-2 text-lg font-bold text-red-400">{fmt(ligandoMov.monto)}</div>
                        </div>
                        ...
                        <div className="flex gap-2">
                            <div className="relative flex-1">
                                ...
                                <input value={searchCfdiPanel} onChange={e => setSearchCfdiPanel(e.target.value)}
                                    ...
                                    className="w-full pl-9 pr-3 py-2 bg-zinc-900 border border-red-500/20 rounded font-mono text-sm text-white focus:outline-none" />
                            </div>
                        ...
                        {loadingCfdisDisp ? (
                            ...
                            <p className="font-mono text-xs text-gray-600 text-center py-4">No se encontraron CFDIs recibidos</p>
                        ) : (
                            <div className="space-y-2">
                                {cfdisFiltrados.map(c => (
                                    <div key={c.id} className="bg-zinc-900/60 border border-red-500/20 hover:border-red-500/40 rounded-lg p-3 transition-colors">
                                        <div className="flex items-start justify-between gap-3">
                                            ...
                                            <div className="text-right shrink-0">
                                                <div className="font-mono text-sm text-red-400">{fmt(c.total)}</div>
